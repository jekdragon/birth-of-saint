...
> Дата: 2026-08-02
> Доп. материалы: UI-SPEC-PDF-REFERENCE.md (концепция от Gemini)

---

## 0. ПРИНЦИПЫ ИЗ PDF (UI-SPEC-PDF-REFERENCE.md)

Адаптировано под наш стек (1024x768, pygame-ce, WASM):

- **UIElement базовый класс** — rect + is_hovered + handle_event/update/draw
- **UIManager** — централизованный менеджер компонентов (делегирует в SceneManager)
- **Focus Management** — геймпад/клавиатурная навигация с подсветкой фокуса
- **Пиксельный контур** — кнопки/панели с 1-2px рамкой, без anti-aliasing
- **Hover-эффект** — смещение на 1-2px + подсветка цвета
- **Pressed-эффект** — смещение вниз на 1px + затемнение
- **Pixelate Fade** — переход с пикселизацией (pygame.transform.scale down → up)
- **Wipe Transition** — пиксельная "штора" (горизонтальная/вертикальная)
- **Dialogue Box** — типewriter-эффект для lore/описаний (0.03с/символ)
- **9-slice scaling** — для рамок панелей (опционально, можно на procedural)
- **Звуки UI** — hover=expand, click=tune, open/close=chat_spark (8-bit)

### КРИТИЧЕСКИ НОВЫЕ РЕШЕНИЯ (из PDF, которых нет в текущем проекте):

1. **Virtual Canvas + Integer Scaling** — рендер на 480x270, потом scale x2 = 960x540 (вписывается в 1024x768 с letterbox). Настоящий пиксель-арт лук.
2. **Пиксельный шрифт** — .ttf (Press Start 2P / Silkscreen) + antialias=False. Вместо дефолтного pygame шрифта.
3. **9-Slice Scaling** — рамки панелей из спрайтов (углы + стороны + центр) вместо draw.rect.
4. **Счётчик сердечек** — discrete HP display: каждое сердечко = N HP, половинки для дробных.
...
7. **Pixelate Fade** — переход через downscale → upscale = пикселизация.
8. **Wipe Transition** — пиксельная штора (горизонтальная/вертикальная).
9. **Tooltip с задержкой** — 0.5с пауза, затем всплывающее окно с характеристиками.
10. **Limited Palette** — PICO-8 (16 цветов) или JUGGLER 64 для единого стиля.

НЕ адаптируем: Integer Scaling на весь экран (у нас уже 1024x768, но VIRTUAL CANVAS — это другое).
